...
const Filter: FC<IFilter> = ({ items }) => {
  const weakItemsCount = items.reduce((count: number, item: IItem) => (
    itemHasWeakPassword(item) ? (count + 1) : count
  ), 0)

  const reusedItemsCount = items.reduce((count: number, item: IItem) => (
    itemHasReusedPassword(item, items) ? (count + 1) : count
  ), 0)

  const oldItemsCount = items.reduce((count: number, item: IItem) => (
